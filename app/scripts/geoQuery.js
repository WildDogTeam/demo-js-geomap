 ...
 * @constructor
 * @this {GeoQuery}
 * @param {wilddog} wilddogRef A wilddog reference.
...
var GeoQuery = function (wilddogRef, queryCriteria) {
  ...
  /**
   * Removes unnecessary wilddog queries which are currently being queried.
   */
   ...
   *
   * @param {wilddog DataSnapshot} locationDataSnapshot A snapshot of the data stored for this location.
   */
  ...
  /**
   * Attaches listeners to wilddog which track when new geohashes are added within this query's
   * bounding box.
   */
  function _listenForNewGeohashes() {
    ...
    geohashesToQuery.forEach(function(toQueryStr) {
      ...
      // Note that wilddog fires the "value" event after every "child_added" event fires.
  ...
  this.cancel = function () {
    ...
    // Turn off all wilddog listeners for the current geohashes being queried
    var keys = Object.keys(_currentGeohashesQueried);
    var numKeys = keys.length;
    for (var i = 0; i < numKeys; ++i) {
      var geohashQueryStr = keys[i];
  ...
  /*  CONSTRUCTOR  */
  /*****************/
  // wilddog reference of the GeoFire which created this query
